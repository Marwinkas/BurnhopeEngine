...
namespace burnhope {
    ...
    public:
        ...
        void Draw(UIContext& context) override {
            ...
            ImGui::SameLine();
            if (ImGui::Button("Unparent", ImVec2(80, 25)) && context.selectedEntity != flecs::entity::null()) {
                context.SaveState();
            ...
            context.registry->each([&](flecs::entity entity, IDComponent& idc, TagComponent& tag) {
                bool isRoot = true;
                if (entity.has<HierarchyComponent>()) {
                    const HierarchyComponent hc = entity.get<HierarchyComponent>();
                    if (hc.parentID != 0) isRoot = false;
                }
    ...
    // Проверяем, есть ли компонент, и получаем его как ссылку (или авто-константную ссылку)
    ...
    if (entity.has<HierarchyComponent>()) {
        ...
        isLeaf = hc.childrenIDs.empty(); // Работаем через точку, так как это объект/ссылка
    }
    ...
    // Сравнение сущностей во Flecs работает напрямую
    if (context.selectedEntity == entity) flags |= ImGuiTreeNodeFlags_Selected;
    if (isLeaf) flags |= ImGuiTreeNodeFlags_Leaf;

    // В качестве ID для ImGui используем внутренний 64-битный ID сущности Flecs
    ...
    if (nodeOpen) {
        if (!isLeaf) {
            if (entity.has<HierarchyComponent>()) {
                auto hc = entity.get<HierarchyComponent>();
                // Копируем вектор айдишников, чтобы безопасно итерироваться
                auto children = hc.childrenIDs; 
                for (uint64_t childID : children) {
                    flecs::entity childEnt = context.FindEntityByID(childID);
                    if (childEnt != flecs::entity::null()) {
                        DrawNode(context, childEnt);
                    }
                }
            }
        }
