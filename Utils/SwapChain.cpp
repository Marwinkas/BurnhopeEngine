...
namespace burnhope
{
  std::string vkResultToString(VkResult result) {
    switch (result) {
        ...
        case VK_ERROR_DEVICE_LOST:          return "VK_ERROR_DEVICE_LOST (Драйвер упал/TDR)";
        case VK_ERROR_SURFACE_LOST_KHR:     return "VK_ERROR_SURFACE_LOST_KHR";
        // Добавьте другие нужные коды по вкусу
        default: return "Unknown VkResult (" + std::to_string(result) + ")";
    }
}

  BurnhopeSwapChain::BurnhopeSwapChain(BurnhopeDevice &deviceRef, VkExtent2D extent)
      : device{deviceRef}, windowExtent{extent}
  {
    init();
  }
  BurnhopeSwapChain::BurnhopeSwapChain(
      BurnhopeDevice &deviceRef, VkExtent2D extent, std::shared_ptr<BurnhopeSwapChain> previous)
  ...
  VkResult BurnhopeSwapChain::acquireNextImage(uint32_t *imageIndex)
  {
    vkWaitForFences(
        device.device(),
        1,
  ...
  VkResult BurnhopeSwapChain::submitCommandBuffers(const VkCommandBuffer *buffers, uint32_t *imageIndex)
  {
    ...
    VkResult results = vkQueueSubmit(device.graphicsQueue(), 1, &submitInfo, inFlightFences[currentFrame]);
    if (results !=
        VK_SUCCESS)
    {
      throw std::runtime_error("failed to submit draw command buffer! Error code: " + vkResultToString(results));
    }
  ...
  VkFormat BurnhopeSwapChain::findDepthFormat()
  {
