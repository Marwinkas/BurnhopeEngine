...
void SavePostProcessSettings() {
    ...
    glm::vec3 GetMouseRay(BurnhopeWindow& window, Camera& camera) {
        double mouseX, mouseY; glfwGetCursorPos(window.getGLFWwindow(), &mouseX, &mouseY);
        auto extent = window.getExtent();
       
        float x = (2.0f * (float)mouseX) / extent.width - 1.0f; float y = 1.0f - (2.0f * (float)mouseY) / extent.height;
    ...
    void Draw(BurnhopeWindow& window, Camera& camera, entt::registry& registry, VkCommandBuffer commandBuffer) {
        ...
        ImGuizmo::BeginFrame(); ImGuizmo::SetOrthographic(false); ImGuizmo::SetDrawlist(ImGui::GetForegroundDrawList()); auto extent = window.getExtent();
