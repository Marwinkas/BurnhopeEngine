        ...
        void Draw(UIContext& context) override {
            ...
            if (currentMat->isPacking) {
                ...
                if (ImGui::BeginTable("CardsTable", 2, ImGuiTableFlags_SizingStretchProp)) {
                    ...
                    ImGui::TableNextColumn();
                    DrawMaterialProperty("Metallic", currentMat->metallicMap, currentMat->metallicPath, context, &currentMat->metallicStrength, 0.0f, 1.0f, nullptr, false, false, true, nullptr, nullptr, changed, makeSetter([&](const std::string& p){ if(p.empty()) currentMat->setMetallic(nullptr, ""); else currentMat->setMetallic(BurnhopeTexture::createDataTextureFromFile(*context.device, p), p); }, currentMat->metallicMap));
                    ImGui::TableNextColumn();
                    DrawMaterialProperty("Roughness", currentMat->roughnessMap, currentMat->roughnessPath, context, &currentMat->roughnessStrength, 0.0f, 1.0f, nullptr, false, false, true, nullptr, nullptr, changed, makeSetter([&](const std::string& p){ if(p.empty()) currentMat->setRoughness(nullptr, ""); else currentMat->setRoughness(BurnhopeTexture::createDataTextureFromFile(*context.device, p), p); }, currentMat->roughnessMap));
                    ...
                    ImGui::TableNextColumn();
                    DrawMaterialProperty("Ambient Occlusion", currentMat->aoMap, currentMat->aoPath, context, &currentMat->aoStrength, 0.0f, 1.0f, nullptr, false, false, true, nullptr, nullptr, changed, makeSetter([&](const std::string& p){ if(p.empty()) currentMat->setAO(nullptr, ""); else currentMat->setAO(BurnhopeTexture::createDataTextureFromFile(*context.device, p), p); }, currentMat->aoMap));
                    ImGui::TableNextColumn();
                    DrawMaterialProperty("ORM Map (AO/Rough/Metal)", currentMat->ormMap, currentMat->ormPath, context, nullptr, 0.0f, 0.0f, nullptr, false, false, true, nullptr, nullptr, changed, makeSetter([&](const std::string& p){ if(p.empty()) currentMat->setORM(nullptr, ""); else currentMat->setORM(BurnhopeTexture::createDataTextureFromFile(*context.device, p), p); }, currentMat->ormMap));
