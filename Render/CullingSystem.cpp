...
void CullingSystem::createPipeline() {
    ...
    auto layout = cullLayout->getDescriptorSetLayout();
    ...
    if (vkCreateShaderModule(device.device(), &shaderInfo, nullptr, &shaderModule) != VK_SUCCESS)
        throw std::runtime_error("Failed to create culling shader module!");
                                    ...
                                    uint32_t objectCount)
{
    ...
    vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_COMPUTE,
        cullPipelineLayout, 0, 1, &cullSet, 0, nullptr);
    ...
    push.zNear        = 0.01f;
    for (int i = 0; i < 6; i++) push.frustumPlanes[i] = planes[i];
