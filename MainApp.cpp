    ...
    for(int i = 0; i < 4; i++) ubo.sunLightSpaceMatrices[i] = cascadeMats[i];
    ...
    renderGraph.addPass("Shadow Maps Pass", shadowBarriersBegin, [&](VkCommandBuffer cmd) {
        ...
        for (int i = 0; i < BurnhopeCSM::CASCADE_COUNT; i++) {
            ...
            shadowRenderSystem->renderShadow(cmd, cascadeMats[i], registry, shadowObjectSet);
        ...
        for (auto entity : lightView) {
            ...
            if (light.type == LightType::Spot) {
                ...
                shadowRenderSystem->renderShadow(cmd, light.lightSpaceMatrix, registry, shadowObjectSet);
            }
                ...
                for (int face = 0; face < 6; face++) {
                    ...
                    shadowRenderSystem->renderShadow(cmd, faceMatrix, registry, shadowObjectSet);
                }
            }
        }
        vkCmdEndRenderPass(cmd);
    ...
    renderGraph.addPass("Radiance Cascades GI", rcBarriers, [&](VkCommandBuffer cmd) {
        // AABB сцены — задай под свою сцену или вычисляй динамически
