...
struct MaterialData {
    ...
    vec2 uvScale;
    vec2 padding; // Добавлено выравнивание как в OpenGL
};
...
void main() {
    ...
    // 1. Gram-Schmidt ортогонализация (Перенесено из OpenGL)
    ...
    // 2. Parallax / Height (Перенесено двойное чтение из OpenGL)
    if (mat.hasHeight == 1) {
        ...
        // Второе чтение по смещенным UV!
        height  = texture(allTextures[nonuniformEXT(mat.heightIdx)], finalUV).r; 
    }

    // 3. Чтение базовых PBR карт
    if (mat.hasAlbedo == 1)    albedo    = texture(allTextures[nonuniformEXT(mat.albedoIdx)], finalUV).rgb;
    ...
    // 4. Реконструкция нормали из двух каналов RG (Перенесено из OpenGL)
    if (mat.hasNormal == 1) {
